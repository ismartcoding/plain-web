...
describe('SMS send schema compatibility', () => {
  ...
  it('passes the optimistic pending ID as a per-send request ID', async () => {
    mockFetch.mockResolvedValue({ data: { sendSms: true } })

    await expect(sendSmsWithCompatibility({
      number: '+15551234567', body: 'hello', subscriptionId: 1, requestId: 'pending-a',
    })).resolves.toEqual({ ok: true })

    expect(mockFetch).toHaveBeenCalledOnce()
    expect(mockFetch.mock.calls[0][1]).toMatchObject({ requestId: 'pending-a' })
  })

  it('retries the legacy mutation only for an unknown requestId argument', async () => {
    mockFetch
      .mockResolvedValueOnce({ data: null, errors: [{ message: 'Unknown argument "requestId" on field "sendSms"' }] })
    ...
    expect(mockFetch.mock.calls[1][1]).not.toHaveProperty('requestId')
  })
