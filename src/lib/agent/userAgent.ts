...
export function getLegacyAgent(userAgent?: string): AgentInfo {
  ...
  const { preset: browserPreset, version: browserVersion } = findPreset(BROWSER_PRESETS, nextAgent)

  const { preset: osPreset, version: osVersion } = findPreset(OS_PRESETS, nextAgent)
