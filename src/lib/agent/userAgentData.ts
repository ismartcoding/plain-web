import type { UADataValues, AgentInfo, AgentBrowserInfo, AgentOSInfo } from './types'
import { findBrand, convertVersion, findPresetBrand, getUserAgentString } from './utils'
...
export function getClientHintsAgent(osData?: UADataValues): AgentInfo {
  ...
  browser.webkit = !browser.chromium && WEBKIT_PRESETS.some((preset) => findBrand(brands, preset))
  ...
  const platfomResult = OS_PRESETS.find((preset) => {
