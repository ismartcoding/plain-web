export default {
  app_name: 'PlainApp',
  or: '或',
  hide: '隐藏',
  link_copied: '链接已复制',
  copy_failed: '复制失败',
  copy_link: '复制链接',
  unwrap: '不换行',
  wrap: '换行',
  current_folder: '当前目录',
  folder: '文件夹',
  trash_tips: '回收站的项目将在30天后永久删除。',
  x_items: '{count} 项目 | {count} 项目',
  export_notes: '导出笔记',
  in_progress: '进行中',
  completed: '已完成',
  types: '类型',
  save_to_notes: '保存到笔记',
  x_selected: '已选择{count}项',
  card_view: '卡片视图',
  grid_view: '网格视图',
  today: '今天',
  yesterday: '昨天',
  back: '返回',
  resume: '继续',
  select_all: '全选',
  troubleshoot: '排查问题',
  fix_disconnect_tips: '连接已断开，请尝试保持手机屏幕亮起或将手机连接 USB 充电。或重新打开应用并刷新页面。',
  failed_connect_ws: '连接已断开，请重新启动应用程序并重试。',
  desktop_notification_need_https: '桌面通知需要安全链接（HTTPS）。',
  use_https_link: '使用HTTPS链接',
  send: '发送',
  export: '导出',
  date: '日期',
  grant_permission: '授权',
  desktop_notification_permission_not_granted: '未授予桌面通知权限。',
  desktop_notification_permission_denied: '通知权限已被拒绝，请在"系统设置 > 通知"中手动开启。',
  notification_open_settings: '打开系统设置',
  desktop_notification_permission_grant_title: '授予桌面通知权限',
  desktop_notification_permission_grant_message: '1. 点击地址栏中的挂锁图标。\n2. 打开"站点设置"或"权限"。\n3. 找到"通知"并将其更改为"允许"。',
  sort: '排序',
  sort_by: {
    date_asc: '最旧日期优先',
    date_desc: '最新日期优先',
    size_asc: '最小优先',
    size_desc: '最大优先',
    name_asc: '名称（从A到Z）',
    name_desc: '名称（从Z到A）',
  },
  group_by: {
    label: '分组方式',
    none: '无',
    taken_time: '拍摄时间',
  },
  paging: {
    label: '翻页方式',
    number: '页码翻页',
    scroll: '无限滚动',
  },
  view_options: '显示选项',
  print: '打印',
  cancel: '取消',
  ok: '好的',
  connection_timeout: '连接超时',
  save: '保存',
  saving: '保存中',
  saved: '已保存',
  status: '状态',
  enabled: '启用',
  updated: '已更新',
  enable: '启用',
  deleted: '已删除',
  create: '创建',
  edit: '编辑',
  edit_image: '编辑图片',
  delete: '删除',
  switch: '切换',
  actions: '操作',
  running: '运行中',
  stopped: '停止',
  basic: '基本',
  name: '名称',
  address: '地址',
  yes: '是',
  no: '否',
  confirm_to_delete_name: '确认删除"{name}"?',
  confirm_to_delete: '您确定要删除这{count}个项目吗？此操作无法撤销。',
  valid: {
    string_min: '至少{min}个字符',
    required: '字段必填',
  },
  theme: {
    title: '主题',
    system: '系统默认',
    light: '浅色',
    dark: '黑暗',
  },
  language: {
    title: '语言',
    system: '系统默认',
  },
  header_actions: {
    language: '更改语言',
    theme: '更改主题',
    uploads: '上传列表',
    audios: '音频',
    logout: '退出登录',
    notifications: '通知',
  },
  quick_actions: '快捷操作',
  page_title: {
    home: '首页',
    messages: '短信',
    conversations: '会话',
    contacts: '联系人',
    calls: '通话记录',
    apps: '应用',
    files: '文件',
    audios: '音频',
    videos: '视频',
    images: '图片',
    books: '书籍',
    feeds: '订阅',
    notes: '笔记',
    json_viewer: 'JSON查看器',
    qrcode_generator: '二维码生成器',
    trash: '回收站',
    screen_mirror: '屏幕镜像',
    device_info: '设备信息',
    docs: '文档',
    chat: '聊天',
    local_chat: '本机聊天',
    developer: '开发者',
    image_editor: '图片编辑器',
  },
  developer: {
    storage: '存储',
    datastore: 'DataStore',
    database: '数据库',
    logs: '日志',
    developer_mode: '开发者模式',
    clear_logs: '清空日志',
    key: '键',
    value: '值',
    db_row_count: '{count} 条记录',
  },
  online: '在线',
  offline: '离线',
  unknown: '未知',
  created_at: '创建时间',
  updated_at: '更新时间',
  active_at: '活跃时间',
  rename: '重命名',
  description: '描述',
  notes: '备注',
  invalid_value: '输入有误',
  space: '',
  creator: '创建者',
  content: '内容',
  from: '来自',
  type: '类型',
  time: '时间',
  no_permission: '没有权限，请到手机应用 > "访问设置"里面打开权限并刷新页面重试。',
  check_phone: '请在手机上查看操作。',
  open_access_settings: '打开 访问设置',
  no_data: '没有数据。',
  load_more: '加载更多',
  no_more_data: '没有更多数据',
  loading: '加载中...',
  duration: '时长',
  start_time: '开始时间',
  apply: '应用',
  started_at: '开始时间',
  preview: '预览',
  editor_view: '编辑器',
  split_view: '分屏',
  write_markdown: '输入 Markdown...',
  md_h1: '标题 1',
  md_bold: '粗体',
  md_italic: '斜体',
  md_strike: '删除线',
  md_inline_code: '行内代码',
  md_link: '链接',
  open_image: '查看图片',
  copy_image_link: '复制图片链接',
  copy_code: '复制代码',
  md_h2: '标题 2',
  md_h3: '标题 3',
  md_ul: '无序列表',
  md_ol: '有序列表',
  md_task: '待办列表',
  md_quote: '引用',
  md_code: '代码块',
  md_table: '表格',
  md_hr: '分割线',
  md_image: '图片',
  uploading: '上传中...',
  view: '查看',
  copy: '复制',
  paste: '粘贴',
  cut: '剪切',
  duplicate: '创建副本',
  properties: '属性',
  modified_at: '修改时间',
  path: '路径',
  open: '打开',
  no_task: '暂无任务',
  load_failed: '加载{name}失败',
  network_error: '网络异常，无法连接API',
  desktop_access_disabled: '电脑访问功能已关闭。请在手机上打开 PlainApp，并开启"PlainApp 服务"。',
  select_first: '请先选择！',
  clear_list: '清空列表',
  tools: '工具',
  json_viewer: 'JSON查看器',
  qrcode_generator: '二维码生成器',
  title: '标题',
  restore: '恢复',
  in_trash: '在回收站',
  not_in_trash: '不在回收站',
  trash: '回收站',
  move_to_trash: '移到回收站',
  work: '工作',
  expand_all: '展开全部',
  collapse_all: '合并全部',
  all: '全部',
  manage: '管理',
  manage_devices: '管理设备',
  add: '添加',
  confirm: '确认',
  no_files: '没有文件',
  size: '大小',
  try_again: '重试',
  deleting: '删除中',
  sending: '发送中…',
  downloading: '下载中',
  waiting: '等待中',
  paused: '已暂停',
  failed: '失败',
  color: '颜色',
  pause: '暂停',
  refresh: '刷新',
  retry: '重试',
  shared_files: '共享文件',
  share_read_only: '只读',
  share_expires_on: '有效期至 {date}',
  share_empty: '没有文件',
  share_empty_tip: '该文件夹中没有内容。',
  share_error_invalid_link_title: '链接无效',
  share_error_invalid_link_tip: '该链接不完整，无法打开。',
  share_error_unauthorized_title: '链接无效',
  share_error_unauthorized_tip: '链接令牌无效。',
  share_error_forbidden_title: '无法访问',
  share_error_forbidden_tip: '您没有权限打开此共享。',
  share_error_expired_title: '共享已过期',
  share_error_expired_tip: '此共享链接已过期。',
  share_error_not_found_title: '共享不存在',
  share_error_not_found_tip: '该共享可能已被删除。',
  share_error_password_required_title: '需要密码',
  share_error_password_required_tip: '此共享受密码保护。',
  share_error_network_title: '连接失败',
  share_error_network_tip: '无法连接设备，请稍后重试。',
  share_error_error_title: '发生错误',
  share_error_error_tip: '加载共享失败，请重试。',
  remove: '移除',
  info: '信息',
  other: '其他',
  zoom_in: '放大',
  zoom_out: '缩小',
  resize: '调整大小',
  rotate_left: '左旋转',
  rotate_right: '右旋转',
  close: '关闭',
  close_other_tabs: '关闭其他标签页',
  close_tabs_to_the_right: '关闭右侧标签页',
  relaunch_app: '重启应用',
  all_items_on_this_page_are_selected: '已选择此页面上{count}项。',
  all_items_are_selected: '已选择全部{count}项。',
  select_all_items: '选择所有{count}项',
  clear_selection: '清除选择',
  unsaved_changes_warning: '你有未保存的修改。确定要退出吗？',
  keyboard_shortcuts: '键盘快捷键',
  delete_selected: '删除所选',
  navigate_pages: '浏览页面',
  range_select: '范围选择',
  previous_next: '上一个/下一个',
  close_preview: '关闭预览',
  settings: '设置',
  start: '开始',
  stop: '停止',
  test: '测试',
  increase_font_size: '增大字体',
  decrease_font_size: '减小字体',
  font_size: '字体大小',
  reset_font_size: '重置字体大小',
  removed: '已删除',
  added: '已添加',
  favorites: '收藏夹',
  add_to_favorites: '添加到收藏夹',
  remove_from_favorites: '从收藏夹中移除',
  settings_saved: '设置已保存',
  notification_sound_on: '通知声音已开启',
  notification_sound_off: '通知声音已关闭',
  customize_ui: '自定义界面',
  customize_ui_tab_sidebar: '侧边栏',
  customize_ui_tab_home: '首页',
  drag_to_reorder: '拖拽排序',
  exclude_directories: '排除目录',
  excluded_dirs_hint: '这些目录中的文件将在媒体视图中隐藏。',
  add_excluded_dir: '添加目录',
  no_excluded_dirs: '没有排除的目录。',
  home: {
    connect_device: '连接设备',
    connect_device_desc: '连接手机以访问文件、媒体等内容。',
    local_mode: '本地',
    server_card_title: '本地服务器',
    server_http: 'HTTP',
    server_https: 'HTTPS',
    server_port: '端口',
    server_running: '运行中',
    server_stopped: '已停止',
    server_copy_http_url: '复制 HTTP 地址',
    server_copy_https_url: '复制 HTTPS 地址',
    server_url_copied: '地址已复制',
  },
}
